...
if(f!=NULL)
	{
	    ...
	    {
	    printf("Dimentsioak agertu egin behar du eta 1 edo handiagoa izan behar du\n");
	    return (*dptr);
	    }
	}
      else
...
if(f!=NULL)
    ...
    while(fgets(lerroa, MAX_BUF, f) != NULL)
        {
        if((lerroa[0] != '#') && (lerroa[0] != '\0') && (lerroa[0] != 'd'))
            {
              ...
              else
		{
		  ...
		  else printf("KONTUZ!: dimentsioa %d dela esan arren datu gehiago ageri dira fitxategian\n", *dptr);
		}
            }
        }
    ...
    if(kont<*dptr)
        {
	printf("ERROREA!: dimentsia %d dela esan arren %d balio besterik ez dira ageri fitxategian\n",*dptr, kont);
        return(-kont);
        }
    fclose(f);
    }
